...
import useExportCustomerList from "@/hooks/query/useExportCustomerList";
import { useDateSelection } from "@/hooks/utils/useDateSelection";
import { ExportDialogParam } from "@/lib/types";
import { exportCustomerListSchema } from "@/schema/all-export-schema";
...
type ExportCustomerListFormData = z.infer<typeof exportCustomerListSchema>;

export function ExportCustomerListDialog({ open, onOpenChange }: Readonly<ExportDialogParam>) {
  const { handleExportCustomerList, isSuccess, isPending, reset } = useExportCustomerList();

  const form = useForm<ExportCustomerListFormData>({
    resolver: zodResolver(exportCustomerListSchema),
    defaultValues: {
      customerName: "",
      customerEmail: "",
      customerPhoneNo: "",
  ...
  const handleSubmit = (formData: ExportCustomerListFormData) => {
    ...
    const payload = {
      ...
      ...(formData.customerPhoneNo ? { customerPhoneNo: formData.customerPhoneNo } : {}),
      ...(formData.customerId ? { customerId: formData.customerId } : {}),
    };

    handleExportCustomerList(payload);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Export Customer List</DialogTitle>
        </DialogHeader>
