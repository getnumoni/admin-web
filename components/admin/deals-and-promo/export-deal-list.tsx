...
import useExportDealList from "@/hooks/query/useExportDealList";
import { useDateSelection } from "@/hooks/utils/useDateSelection";
import { ExportDialogParam } from "@/lib/types";
import { exportDealListSchema } from "@/schema/all-export-schema";
...
type ExportDealListFormData = z.infer<typeof exportDealListSchema>;

export function ExportDealListDialog({ open, onOpenChange }: Readonly<ExportDialogParam>) {
  const { handleExportDealList, isSuccess, isPending, reset } = useExportDealList();


  const form = useForm<ExportDealListFormData>({
    resolver: zodResolver(exportDealListSchema),
    defaultValues: {
      status: "",
      startDate: new Date(),
      endDate: new Date(),
  ...
  const handleSubmit = (formData: ExportDealListFormData) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Export Deal List</DialogTitle>
        </DialogHeader>
